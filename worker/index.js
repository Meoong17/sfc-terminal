...
// ── Batas stream SSE (audit G11, 2026-09-18) ──
// Best-effort per isolate: Cloudflare menjalankan banyak isolate, jadi angka ini BUKAN
// kuota global — tujuannya mencegah satu klien membuka ribuan koneksi di isolate yang sama.
// Rate-limit WAF di dashboard Cloudflare tetap disarankan sebagai lapisan sebenarnya.
let SSE_ACTIVE = 0;
const MAX_SSE_STREAMS = 60;
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path === '/events') {
      // Audit 2026-09-18 (G11): endpoint publik tanpa batas apa pun.
      // Pagar 1 — tolak klien yang datang dari halaman origin lain (Origin ada tapi bukan allowlist).
      //   Catatan: EventSource same-origin tidak mengirim Origin, jadi absent = diizinkan.
      // Pagar 2 — batasi jumlah stream konkuren per isolate agar satu klien tidak membuka ribuan.
      //   Rate-limit WAF Cloudflare tetap disarankan (di luar repo).
      ...
      SSE_ACTIVE++;
      ctx.waitUntil(pollAndStream().finally(() => { SSE_ACTIVE = Math.max(0, SSE_ACTIVE - 1); }));
    ...
    // Audit 2026-09-18 (G10): '/app.js' dihapus dari daftar — berkas itu pensiun (index.html tidak
    // mereferensikannya) sehingga hanya menghasilkan 404 yang diklaim Worker.
    ...
    // Ikon PWA (audit G10) — biner: jangan lewat jalur teks di atas (akan merusak PNG).
    if (path.startsWith('/icons/') && path.endsWith('.png')) {
      const resp = await fetchAny(env, path, 'image/png');
      if (!resp) return new Response('Not Found', { status: 404 });
    ...
    // Aset tidak dikenal → 404 tegas (audit G10): catch-all SPA dulu membalas `200 text/html`
    // untuk path seperti /icon-192.png atau /app.js sehingga 404 tersamar sebagai halaman valid.
    if (/\.(js|json|png|jpe?g|svg|webp|ico|css|woff2?|map|txt|xml)$/i.test(path)) {
