...
# Compute from actual ML performance data
bt_sharpe = None
bt_max_dd = None
bt_win_rate = None
bt_return = None
...
try:
    ...
    # Signal stability from method agreement
    bt_stability = round(method_agreement * 0.6 + (1.0 - abs(transition_risk - 0.5)), 3)
    bt_stability = max(0.1, min(1.0, bt_stability))
    
    print(f"  [Backtest] Sharpe={bt_sharpe} | WinRate={bt_win_rate:.0%} | "
          f"MaxDD={bt_max_dd:.1%} | Stability={bt_stability:.2f} | "
          f"Periods={bt_periods}", file=sys.stderr)
except Exception as e:
